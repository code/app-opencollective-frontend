...
import { Account, Currency as CurrencyEnum, SocialLinkType } from '../../lib/graphql/types/v2/graphql';
  ...
  z
    .object({
      ...
      slug: z
        ...
        .regex(
          /^[\w-]+$/,
          intl.formatMessage({ defaultMessage: 'Slug can only contain letters, numbers, hyphens and underscores' }),
        ),
    ...
    .superRefine((values, context) => {
      ...
      if (!values.legalName && !values.name) {
        context.addIssue({
          ...
          message: intl.formatMessage({ defaultMessage: 'At least one of "Display name" or "Legal name" is required' }),
          path: ['name'],
        });
      }
    });
...
export const EditAccountInfoForm = ({ accountSlug }) => {
  ...
  return (
    <FormikZod initialValues={getInitialValues(account)} schema={accountInfoFormSchema} onSubmit={console.log}>
